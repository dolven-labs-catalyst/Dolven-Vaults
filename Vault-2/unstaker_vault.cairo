...
@storage_var
func totalLockedValue() -> (tvl: felt) {
}
...
@event
func Unlocked(user_account: felt, amount: felt, timestamp: felt, _nonce: felt) {
}
@event
func Locked(user_account: felt, amount: felt, timestamp: felt, _nonce: felt) {
}
...
@view
func get_tvl{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (res: felt) {
    let tvl_to_be_unstaked: felt = totalLockedValue.read();
...
func lockTokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    user_address: felt, _amount: felt, lockType_: felt
) {
    alloc_locals;
    onlyStakingContract();
    ReentrancyGuard._start();
    ...
    let is_amount_bigger_than_zero: felt = is_le(1, _amount);
    with_attr error_message("lockTokens::amount is less than or equals to zero") {
        assert is_amount_bigger_than_zero = 1;
    }
    let amount_uint : Uint256 = felt_to_uint256(_amount);
    let (is_tx_success: felt) = IERC20.transferFrom(_stakingToken, msg_sender, this, amount_uint);
    ...
    let old_tvl: felt = totalLockedValue.read();
    let new_tvl: felt = old_tvl + _amount;
...
func cancelLock{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let lock_amount_uint : Uint256 = felt_to_uint256(lock_details.amount);
    IERC20.approve(staking_token, stake_contract, lock_amount_uint);
    ...
    let old_tvl: felt = totalLockedValue.read();
    let new_tvl: felt = old_tvl - lock_details.amount;
    totalLockedValue.write(new_tvl);
...
func unlock{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let lock_amount_uint : Uint256 = felt_to_uint256(lock_details.amount);
    let is_txs_success: felt = IERC20.transfer(_token_address, user_address, lock_amount_uint);
